...
void fast_hadamard_transform_hip(HadamardParamsBase &params, hipStream_t stream);

template<typename input_t>
void fast_hadamard_transform_12N_hip(HadamardParamsBase &params, hipStream_t stream);

template<typename input_t>
void fast_hadamard_transform_20N_hip(HadamardParamsBase &params, hipStream_t stream);

template<typename input_t>
void fast_hadamard_transform_28N_hip(HadamardParamsBase &params, hipStream_t stream);

template<typename input_t>
void fast_hadamard_transform_40N_hip(HadamardParamsBase &params, hipStream_t stream);
...
fast_hadamard_transform(at::Tensor &x, float scale) {
    ...
    at::hip::HIPGuardMasqueradingAsCUDA device_guard{(char)x.get_device()};
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.scalar_type(), "fast_hadamard_transform", [&] {
        fast_hadamard_transform_hip<input_t>(params, stream);
    });
    if (dim_og % 8 != 0) {
...
fast_hadamard_transform_12N(at::Tensor &x, float scale) {
    ...
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.scalar_type(), "fast_hadamard_transform", [&] {
        fast_hadamard_transform_12N_hip<input_t>(params, stream);
    });
    if (dim_og % (4 * 12) != 0) {
        out = out.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, dim_og)});
    }
...
fast_hadamard_transform_20N(at::Tensor &x, float scale) {
    ...
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.scalar_type(), "fast_hadamard_transform", [&] {
        fast_hadamard_transform_20N_hip<input_t>(params, stream);
    });
    if (dim_og % (4 * 20) != 0) {
        out = out.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, dim_og)});
    }
...
fast_hadamard_transform_28N(at::Tensor &x, float scale) {
    ...
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.scalar_type(), "fast_hadamard_transform", [&] {
        fast_hadamard_transform_28N_hip<input_t>(params, stream);
    });
    if (dim_og % (8 * 28) != 0) {
        out = out.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, dim_og)});
    }
...
fast_hadamard_transform_40N(at::Tensor &x, float scale) {
    ...
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.scalar_type(), "fast_hadamard_transform", [&] {
        fast_hadamard_transform_40N_hip<input_t>(params, stream);
    });
    if (dim_og % (8 * 40) != 0) {
        out = out.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, dim_og)});
    }
